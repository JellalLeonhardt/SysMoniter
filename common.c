...
void task_show(proc_t *task){
	...
	show_special(0, fmtmk(TASK_line, task->tid, task->pcpu, task->size, (float)task->size / kb_main_total, task->vm_size, task->euser, task->utime, (unsigned long)task->vm_exe / 1024, (unsigned long)(task->vm_data / 1024 / 1024 + task->vm_stack / 1024 / 1024), task->ppid, task->ruser, task->egroup, task->cmdline));
